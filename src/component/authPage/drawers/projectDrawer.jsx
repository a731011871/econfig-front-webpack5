...
class projectDrawer extends React.PureComponent {
    ...
    fetchProject = async (value = '') => {
        ...
        try {
            ...
            let result = [];
            /**
             * econfig取的项目列表为租户下的项目列表，其他应用取应用下项目列表。
             * 从项目管理中进入授权，项目列表只保留项目管理中的该项目
             * pv不分页，增加全选选项
             * */
                ...
                >
                    {projectList.map(project => (
                        <Option
                            ...
                            {...project}
                            title={`${project.projectSerialNo ? `【${project.projectSerialNo}】` : ''}${project.projectName}`}
                        >
                            {`${project.projectSerialNo ? `【${project.projectSerialNo}】` : ''}${project.projectName}`}
                        </Option>
                    ))}
                </Select>
