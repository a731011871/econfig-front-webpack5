...
class roleProjectSoftPage extends React.PureComponent {
    ...
    componentDidMount = async () => {
        try {
            ...
            if (this.props.authInfoItem.length) {
                projectList = await authServices.getTenantProjects(
                    this.props.authInfoItem[0].projectIds
                );
            }
            const roleList = await $http.get(
                `${urls.getAuthEconfigRoleList}?appId=${this.props.appId}`
            );
            const softList = await $http.get(urls.getSoftList, {
                isIncludeExpired: true,
                isFilterApp: true
            });
            // roleList只有部门用户才可以选择econfig_company_admin，别的情况不显示
            this.setState({
                projectList: projectList.list,
                softList: softList || [],
                roleList: (this.props.userType === 'companyUser'
                    ? roleList
                    : roleList.filter(
                          item => item.id !== 'econfig_company_admin'
                      )
                ).map(item => ({
                    ...item,
    ...
    addSoft = record => {
        const selectSoftIds = record.softIds || [];
        const softList = this.state.softList.filter(
        ...
        drawerFun({
            ...
            compontent: props => (
                <SoftDrawer
                    {...props}
                    intl={this.props.intl}
                    softList={softList.filter(
                        item => item.valid === '1' && item.appId !== 'econfig'
                    )}
                    appId={this.props.appId}
                    selectRole={record}
            ...
            <div className="roleProjectSitePage">
                ...
                {this.state.showRoleProjectSoftModal && (
                    <RoleProjectSoftModal
                        ...
                        appId={this.props.appId}
                        projectId={this.props.projectId}
                        softList={this.state.softList.filter(
                            item =>
                                item.valid === '1' && item.appId !== 'econfig'
                        )}
                        fromProjectManage={this.props.fromProjectManage}
                        selectProjectIds={selectProjectIds}
