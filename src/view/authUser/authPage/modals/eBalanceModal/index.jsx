...
class eBalanceModal extends React.PureComponent {
            ...
            const resultData =
                ...
                this.props.loginUserRole !== 'projectAdmin'
                    ? result.data.concat({
                        ...
                        id: 'ALL',
                        roleVos: this.props.roleList.filter(item => !item.projectIds || !item.projectIds.length)
                    })
                    : result.data;
    ...
    projectChange = async value => {
        try {
            const selectProject = this.state.projectList.filter(item => item.projectId === value);
            const siteList = await authServices.getCspSiteList(value);
            const storageList = await authServices.getCspStorageList(
                value,
                'esupply'
            );
