...
import { find, uniqBy, includes } from 'lodash';
...
export default class AuthTable extends React.PureComponent {
    ...
    //     const { selectAuthInfo, selectedRowKeys } = this.state;
    //     const { roleList } = this.props;
    //     const tableData = this.state.tableData.concat();
    //     tableData.forEach(dataItem => {
    ...
    getHasAuthNum = () => {
        let hasAuthNumber = 0;
        const { tableData, searchKeyWord } = this.state;
        const { roleList } = this.props;
        ...
        filterTableData.forEach(item => {
            // authData 人员当前授权数据
            // 数据中需要筛选掉needDepartment === 1的角色 剩下的角色数>0，数字+1
            if (
                item.authData &&
                item.authData.filter(item =>
                    includes(
